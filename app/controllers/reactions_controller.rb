class ReactionsController < ApplicationController
  ...
  def create
    reaction_params[:user_id] = current_user.id
    @reaction = Reaction.new(reaction_params)

    respond_to do |format|
      if @reaction.save
        format.html { redirect_to post_url(@reaction.comment.post), notice: "Reaction was successfully created." }
  ...
  def delete_my_reactions
    Reaction.where(user: current_user, comment_id: params[:comment_id]).destroy_all
    redirect_to post_url(Comment.find(params[:comment_id]).post), notice: "Reaction was successfully destroyed."
  end
